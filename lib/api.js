...
export const api = {
  ...
  listarPlantillas: (params = {}) => {
    const qs = new URLSearchParams(params).toString();
    return apiFetch(`/plantillas${qs ? `?${qs}` : ""}`);
  },
  obtenerPlantilla: (id) => apiFetch(`/plantillas/${id}`),
  crearPlantilla: (data) => apiFetch("/plantillas", { method: "POST", body: data }),
  actualizarPlantilla: (id, data) => apiFetch(`/plantillas/${id}`, { method: "PUT", body: data }),
  eliminarPlantilla: (id) => apiFetch(`/plantillas/${id}`, { method: "DELETE" }),
};
